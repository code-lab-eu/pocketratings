...
**Backend work must not be marked complete until all checks in this skill (format, Clippy, tests, and coverage) have been run and passed.** Running only a subset (e.g. only tests or only coverage) is not sufficient. See the completion gate in AGENTS.md.
