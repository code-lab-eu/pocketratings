...
<main class="mx-auto max-w-2xl px-4 py-8">
	...
	<h1 class="mb-4 text-2xl font-semibold text-gray-900">Purchases</h1>
	<a
			...
			{#each purchases as purchase (purchase.id)}
				<li class="flex items-center justify-between gap-2 rounded-lg border border-gray-200 bg-white px-4 py-3">
					<div class="min-w-0 flex-1">
						<span class="font-medium">{productMap.get(purchase.product_id)?.name ?? purchase.product_id}</span>
